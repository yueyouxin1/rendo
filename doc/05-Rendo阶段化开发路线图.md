...
4. `Phase 3`：第一个 `Domain Starter` 落地
5. `Phase 4`：`Capability Pack` 机制落地
...
## 6. Phase 3：第一个 `Domain Starter` 落地
...
- 一个高质量可运行的 `Domain Starter`
- 可被强 Agent 继续开发
- 可被用户直接启动

---

## 7. Phase 4：`Capability Pack` 机制落地
...
**当前最重要的不是继续抽象平台，而是把 `Core Starter`、`rendo cli` 和第一个 `Domain Starter` 做出来。**
...
**先把 `Core Starter`、`Rendo CLI`、`Domain Starter` 和 `Pack` 做成，再逐步接入真实 Rendo 能力，最后才让平台闭环从它们的真实使用中长出来。**
