...
- 日期：2026-04-05
- 文档性质：模板体系原则文档
- 目标：统一模板分层、模板身份、目录、命名与继承理解
- 核心结论：**Rendo 当前应围绕“每种一等模板类型都有自己的 `core -> base -> derived` 主干”来组织，而不是再把“唯一 Core Starter”当成全系统底座。**
...
Rendo 需要统一的模板资产体系，而不是把这些资产拆成互相平行、语义混乱的名字堆。
...
## 8. `application-base-starter` 的正确定位

`application-base-starter` 是当前官方 starter base 中最关键的一个。

它应承担：

- 界面型应用 starter 的 canonical base
- 面向大项目演进的稳定目录模型
...
- Starter 是最重要的消费类型，但不是唯一中心
- `application-base-starter` 是当前最关键的 starter base
- 目录、命名、manifest 和 install 语义都应服务于“让强 Agent 无歧义理解模板系统”
