...
def _resolve_bundle_output_path(output_file: str | None, template_id: str, version: str) -> Path:
    safe_template_id = template_id.replace("/", "-").replace("\\", "-")
    return Path(output_file or f"{safe_template_id}-{version}.rendo-bundle.json").resolve()


def _resolve_publish_output_path(output_file: str | None, template_id: str, version: str) -> Path:
    safe_template_id = template_id.replace("/", "-").replace("\\", "-")
    return Path(output_file or f"{safe_template_id}-{version}.rendo-publish.json").resolve()
...
def main() -> None:
    ...
    create_parser.add_argument("--as-template", action="store_true")
    create_parser.add_argument("--template-id")
    create_parser.add_argument("--template-name")
    create_parser.add_argument("--template-title")
