...
def _humanize_template_id(template_id: str) -> str:
    parts = [part for part in template_id.replace("/", "-").replace("_", "-").split("-") if part]
    return " ".join(part.capitalize() for part in parts) or "Derived Template"
...
def persist_workspace_metadata(
    ...
    creation_mode: str,
    local_template_identity: dict,
) -> None:
    manifest = source["manifest"]
    project = validate_project_manifest(read_json(target / "rendo.project.json"))
    project["surfaces"] = selected_surfaces
    project["template"]["id"] = local_template_identity["id"]
    project["template"]["templateRole"] = "derived"
    project["workspaceId"] = build_workspace_id(local_template_identity["id"], project["projectName"], project["template"]["createdAt"])
    project["origin"] = {
    ...
    template["id"] = local_template_identity["id"]
    template["name"] = local_template_identity["name"]
    template["title"] = local_template_identity["title"]
...
def scaffold_template(
    ...
    *,
    ...
    if as_template and not template_id:
        raise RuntimeError("template workspaces require --template-id")
    local_template_identity = {
        "id": template_id or manifest["id"],
        "name": template_name or _humanize_template_id(template_id or manifest["id"]),
        "title": template_title or template_name or _humanize_template_id(template_id or manifest["id"]),
    }
    replacements = {
        "__RENDO_PROJECT_NAME__": project_name,
        "__RENDO_PROJECT_SLUG__": slugify(project_name),
        "__RENDO_TEMPLATE_ID__": local_template_identity["id"],
        "__RENDO_TEMPLATE_TITLE__": local_template_identity["title"],
    ...
    persist_workspace_metadata(source, target, selected_runtime, selected_surfaces, creation_mode, local_template_identity)
    normalized_copied = [
        relative_path
    ...
    return {
        ...
        "templateId": local_template_identity["id"],
