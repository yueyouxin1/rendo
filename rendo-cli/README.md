...
- every template kind shares the same control-plane skeleton outside that namespace: `.gitignore`, `README.md`, `AGENTS.md`, `CLAUDE.md`, `.agents/`, `docs/`, `interfaces/`, `src/`, `tests/`, `scripts/`, `integration/`
...
Current delivery has two layers:

- the development workspace under [`rendo-cli/`](/D:/code/rendo/rendo-cli)
- the published local executable at [`dist/release/dist/rendo.exe`](/D:/code/rendo/rendo-cli/dist/release/dist/rendo.exe)

The published local executable is now real and usable:

- it resolves packaged runtime assets without depending on the old repo-root `shared/*` layout
- it supports `search / inspect / create / doctor / publish --local`
- `rendo bundle <ref>` still exports formal local bundle artifacts from the internal distribution artifact layer
...
- current tests still prove local parity plus remote fixture-registry compatibility

For local installation into a user-level bin directory:

- `npm run build:release`
- `npm run install:local-cli`

The install script copies `rendo.exe` into `%LOCALAPPDATA%\\Programs\\Rendo\\bin` by default and can append that directory to the user `PATH`.

So the current implementation is:

- usable inside this development repository
- usable as a local published executable for starting derived workspaces such as [`application/saas-starter`](/D:/code/rendo/application/saas-starter)
- still not the final official remote-publish/runtime-integrated distribution model
