...
- `provider/`: kind-specific notes that stay stable across all base and derived descendants
...
This template is the core layer for `provider templates`. Base and derived templates may add more files and stronger opinions, but they should not make the control plane harder to inspect, harder to validate, or harder to inherit.
